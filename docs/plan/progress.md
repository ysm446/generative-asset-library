...
### スニペット項目の複数選択（2026-08-08）

- 項目一覧を Ctrl / Shift クリックで複数選択できるようにし、選択したままファイル一覧へ
  ドラッグして一括移動できるようにした。操作はグリッドの `handleCardClick` に合わせている
  （`snipState.selected` = index の Set、`anchorIndex` は Shift の起点）。
- `entryIndex` は「フォームに出している 1 件」の意味に変えた。複数選択中でもフォームは 1 件だけを
  編集するため、選択（`is-selected`）とフォーム対象（`is-current`、左端のアクセント線）を
  別のクラスで表している。`selectEntry()` は単一選択、`setFormEntry()` はフォームだけの切り替え。
- `moveEntryToFile` → `moveEntriesToFile(indices, target)`。移動先へは元の並び順のまま追記し、
  元ファイルからの削除は index の大きい方から行う（splice で前の index がずれるため）。
- 選択外の項目をドラッグしたときは、その 1 件だけを選択し直してから掴む（一般的な挙動に合わせた）。
- 削除ボタンは選択件数に追従させた（「選択した N 件を削除」）。複数選択中に 1 件だけ消えるのは
  分かりにくいため、選択全体を対象にしている。
- 検証: 実アプリの DOM を取り込み API をスタブしたハーネスをヘッドレス Chrome で実行し、
  単一 / Ctrl / Shift の選択、削除ボタンの表示、DragEvent + DataTransfer による
  ドラッグ移動（移動先の並び順・元ファイルの残り・保存の順序・ステータス表示）の 14 項目を確認した。

### スニペットの自動整理（ローカル LLM）（2026-08-08）

- スニペット編集モードに ✨ ボタンを追加し、`llama-server` にカテゴリ設計と振り分けをさせる。
  仕様は [docs/reference/snippets.md](../reference/snippets.md) にまとめた。
- 設計の判断: **LLM にファイルを書かせない**。`build_plan()` は読み取りだけで整理案（JSON）を返し、
  ユーザーが差分を確認して `apply_plan()` を呼んではじめて書き込む。適用前に
  `data/snippet_backups/<日時>/` へ丸ごとコピーする。
- 2 フェーズに分けたのは `n_ctx`（既定 8192）に収めるため。全 579 件の prefix と description を
  一度に渡すと溢れるので、(1) ファイル一覧＋各 20 件のサンプルでカテゴリ設計 →
  (2) カテゴリを固定して 40 件ずつ振り分け、という形にした。振り分けでは `body` を渡さない。
- `llm_client.chat_json()` を追加した。llama-server の `response_format: json_schema` で JSON を
  強制する（小さいローカルモデルでは、これが無いと出力が壊れて実用にならない）。
  古いビルド用に `json_object` → 素の生成へフォールバックし、`<think>` と ``` フェンスを剥がす。
- LLM が返すファイル名は `_normalize_file_name()` で basename 化 + `[a-z0-9_]` のみに丸める。
  弾くのではなく無害化する方針なので、`../evil` は `evil.code-snippets`（ルート内）になる。
- `apply_plan()` は適用時にファイルを読み直し、**いま実在する `(from, name)` の移動だけ**を実行する。
  案を作った後に手で編集していた場合に、古い案で無関係なファイルを書き戻さないため。
- 書き換え対象になったファイルの `//` コメントは失われる（バックアップには残る）。
  移動が 1 件も無いファイルは触らないので、コメントもそのまま残る。
- 対象ファイルの絞り込み（`include` / `targets_within_selection`）を追加した。
  「人物系は触らず物と背景だけ分け直す」のような使い方のため。**出どころ**（どのファイルの
  中身をバラすか）と**移動先**（どのファイルへ入れてよいか）は別概念なので、既定は両方とも
  選択内に閉じ、チェックを外すと移動先だけ選択外の既存ファイルへ広がる形にした。
  - 移動先を選択内に限るときは、設計されたカテゴリ名が選択外の既存ファイルと一致した場合に
    そのカテゴリを落として警告する（LLM は自由に命名するので、UI の約束を守る歯止めが要る）。
  - 併せて `emptied` の判定を「項目が出ていったファイル」に限定した。以前は元から空の
    ファイルも「空になる」として数えていた。
- 検証: `python tests/test_snippet_organize.py`（`chat_json` をフェイクに差し替え、
  一時フォルダで案の生成 → 適用 → バックアップ → 不正パスの正規化 →
  対象の絞り込み・移動先スコープ・対象外ファイルが手つかずであることまで）。
  FastAPI の TestClient で `/organize/plan`（SSE）と `/organize/apply` の疎通も確認した。
  実モデルでの整理品質と、ダイアログの見た目は未確認。

### サムネイル一覧のフォルダ表示（2026-08-08）

- グリッドの先頭に、選択中フォルダの子フォルダをカードとして並べる。クリックで `selectFolder`
  （ツリーと同じ導線）。ルート以外は先頭に「.. 上へ」カードを出す。
  検索中（`state.query`）はフォルダをまたぐ結果になるため出さない。
- サムネイルは `folders.tree()` の各ノードに追加した `thumb` を使う。
  `index_db.folder_thumbs()` が items を 1 回走査して「フォルダ直下の最新アイテム」を作り、
  `_folder_node` が木をたどって配下フォルダの代表とも比較して最新のものを選ぶ
  （`sort_order` → `created_at` の順で比較）。合わせて `total_count`（配下込みの件数）も返す。
  `item_count` は従来どおり直下のみで、左ツリーの表示は変えていない。
- フォルダカードは `setupFolderDrop` と `showFolderMenu` をそのまま再利用しているので、
  ドロップでの移動も右クリックメニューもツリーと同じ挙動になる。
- 画像カードと混ざるため、`.card` を対象にしていた処理は `.card[data-id]` に絞った
  （矢印キー移動・`updateGridSelection`・`updateGridFavorites`）。フォルダカードには
  `data-rel` を持たせている。
- 検証: 入れ子・空フォルダ・サブフォルダのみのフォルダを作って `tree()` の
  `total_count` / 代表画像の伝播を確認。ヘッドレス Chrome でルート・サブフォルダの表示と、
  フォルダカードのクリックで実際に移動する（DOM に「上へ」カードと移動先の画像が出る）ことを確認した。
