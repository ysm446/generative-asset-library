...
- 左：フォルダツリー（自由な入れ子）。右クリックで作成・リネーム・削除・エクスプローラーで開く・
  画像を一括で出力（出力先の中にフォルダ名のフォルダを作ってコピー。子フォルダを含めるかも選択可。
  完了すると右下に通知が出て、クリックで出力先を開ける）
- 中央：サムネイルグリッド。ドラッグで並べ替え（複数選択のまとめて移動も可）、選択画像に紐づく
  動画・編集画像は下部のストリップに「動画 / 編集」タブで表示。
  グリッドにフォーカスがあるときは矢印キーで選択を移動（←→ で前後、↑↓ で行）
- **一覧にフォルダも表示**：子フォルダが先頭に並び、クリックでそのフォルダへ移動
  （ルート以外は先頭に「.. 上へ」）。サムネイルは中の代表画像（配下含めて最新）で、
  右上に合計件数。右クリックでツリーと同じメニュー、ドロップで画像・フォルダの移動もできる
  （検索中は表示しない）
- **お気に入り**：画像・動画・編集画像のサムネイル右上の★で切り替え（選択中は F キー、複数選択ならまとめて）。
  検索欄の右の★で「お気に入りだけを表示」に絞り込み（★付きの動画・編集画像を持つ画像も対象）。
  ★は `meta.json` に保存され、★付きを持つ画像はカードのバッジが金色になる
- 右：選択に応じたコンテキストパネル（フォルダ→生成 / 画像→プロパティ / 動画→プロパティ）
- 画像 1 件 = 1 フォルダ（`image.png` + `meta.json` + `videos/` + `edits/`）で、**フォルダが正のデータ**。
...
- 未確認の新規画像・動画を含むフォルダには、左のフォルダツリーにも NEW バッジが付く
  （画像は青、動画のみは琥珀色。親フォルダにも表示され、クリックして確認すると消える）
...
- 動画ストリップのサムネイルを右クリックでメニュー（ファイルの場所を開く / 削除。
  複数選択中は選択分をまとめて削除）
- 動画プロパティのプレビューは、下のリピートボタンで繰り返し再生に切り替え（設定は保持）

### 画像編集（スタイル変換など）
- 画像を選んで「画像を編集...」。ComfyUI の画像編集ワークフロー（`workflows/edit/` の JSON）で
  変換し、**その画像に紐づけて保存**する（動画と同じサブアセット。グリッドには出ない）
- 結果は下部ストリップの「編集」タブに並ぶ。クリックでプロパティを開き、プロンプト・設定の編集、
  同じ設定での再生成、削除ができる（★・複数選択・Del キーは動画と同じ操作）
- **元画像と同じ解像度**：チェック（既定オン）で元画像のピクセルサイズのまま出力する。
  チェック中は Width / Height を入力できない。外すと指定した値（空ならワークフローの値）を使う
- **追加の参照画像**：編集パネルの欄にグリッドから画像をドラッグすると、複数の入力画像を取る
  ワークフロー（Qwen Image Edit など）の 2 枚目以降として渡せる。
  ワークフローが受け取れる枚数を超えた分は使われない
- **独立した画像アイテムにする**（昇格）：編集結果をライブラリの画像として複製する。
  昇格すると普通の画像なので、そこから動画生成・検索・シーケンス投入ができる。
  昇格した画像は元画像のすぐ隣に並び、元の編集画像もそのまま残る
- 画像ファイルを「編集」タブにドロップして、外部で作った編集結果を登録することもできる
...
- 画像の Prompt / Negative Prompt では、**スニペット登録済みの語の文字色が変わる**（アクセント色）
  （強調 `(word:1.2)`・アンダースコア・大文字小文字の違いは吸収）。
  マウスオーバーでスニペット名を表示し、ラベル横のパズルアイコンで表示を切り替えられる。
  色の付かない語は右クリックから「スニペットに登録」でその場で追加できる
- Prompt / Negative Prompt の語にマウスを載せると**強調（重み）スライダー**が出る。
  1.0 は括弧なし、1.1 は `(word)`、それ以外は `(word:0.6)` で書き戻す
...
- ✨ ボタンで**ローカル LLM による自動整理**：カテゴリ（＝ファイル）を設計し直し、
  スニペットを振り分ける。**対象にするファイルを選べる**ので、一部だけを整理できる
  （既定では移動先も選択したファイルに限られ、対象外のファイルは変更されない）。結果は差分プレビューで確認でき、項目ごとにチェックを外して除外できる。
...
- ライブラリの動画をノードとして配置し、out→in ポートをドラッグで一本道につなぐ
  （パレットのカードをノードエリアへドラッグ。クリックはプレイヤーでの試聴）
- パレットの動画を既存ノードの上にドロップすると、位置・接続はそのままでクリップだけ差し替え
- パレットのカード右端の★でお気に入りを切り替えられる（ライブラリ側の★と同じ）
- 左の一覧はフォルダ（1 段）で整理可。ツールバーの「新規フォルダ」で作成し、
  ドラッグまたは行の […] メニュー「フォルダへ移動」で振り分け（フォルダ削除時は中身が未分類に戻る）
- パン / ズーム / ドラッグ移動 / エッジ切断 / 順路に沿った連続再生
- ノードエリア右上の「整列」でノードを格子状に並べ直す（範囲選択中は選択したノードだけ）
- F キーで選択ノードにフォーカス、A キーでノード全体を表示
...
| プレフィックス | 主な用途 |
|---|---|
| `/api/library` | フォルダ / アイテム / 動画 / 編集画像の CRUD、編集画像の昇格、取り込み、検索、並べ替え、embedding、root |
