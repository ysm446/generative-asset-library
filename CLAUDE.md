...
- 作業が一区切りしたら [docs/changelog.md](docs/changelog.md) に変更内容を追記する:
  - `## Unreleased / main` の直下に、作業日の見出し `### YYYY-MM-DD` を設けて（同日の見出しが
    あればそこに）ユーザー視点の変更点を「〜しました」の箇条書きで追加する。
  - 内部リファクタリングのみ等、ユーザーに見えない変更は 1 行にまとめてよい。
  - 冒頭の更新日時も現在の作業日時に更新する。
